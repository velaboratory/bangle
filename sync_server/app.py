    ...
    with dbConnection() as con:
        ...
        if complete == 1:
            ...
            if (app_name != target_app_name) or (app_version != target_app_version):
                wants_sync = 1
            con.execute("update device set last_data_sync = ?, wants_sync = ? where id=?",(datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"),wants_sync,device_id ))

            return success({"sync_id": insert_id, "config_json": base64.b64encode(device_df.iloc[0].target_config_json.encode()).decode(), "target_app_name": target_app_name, "target_app_version":target_app_version})
        else:
